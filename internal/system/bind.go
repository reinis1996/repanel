...
func rebuildNamedConf(bindDir string, slaveIPs []string) error {
	...
	for _, e := range entries {
		// DNSSEC marker files (db.<name>.dnssec) live alongside the zones; they
		// flag which zones to sign and must not be treated as zones themselves.
		if e.IsDir() || strings.HasSuffix(e.Name(), ".dnssec") {
			continue
		}
		...
		dnssec := ""
		if _, err := os.Stat(filepath.Join(zonesDir, e.Name()+".dnssec")); err == nil {
			dnssec = fmt.Sprintf(" dnssec-policy default; inline-signing yes; key-directory \"%s\";", keyDir)
		}
		fmt.Fprintf(&sb, "zone \"%s\" { type master; file \"%s\";%s%s };\n",
			name, filepath.Join(zonesDir, e.Name()), dnssec, transfer)
	}
