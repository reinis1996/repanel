...
func (s *Server) rebuildMail() error {
	...
	rows, err = s.DB.Query(`SELECT id, domain_id, source, destination, keep_copy FROM mail_aliases`)
	if err != nil {
		return err
	}
	for rows.Next() {
		...
		var keep int
		if rows.Scan(&a.ID, &a.DomainID, &a.Source, &a.Destination, &keep) == nil {
			a.KeepCopy = keep != 0
	...
	rows, err = s.DB.Query(`SELECT id, domain_id, address, members FROM mail_lists`)
	if err != nil {
		return err
	}
	for rows.Next() {
		var l models.MailList
		var members string
		if rows.Scan(&l.ID, &l.DomainID, &l.Address, &members) == nil {
			l.Members = splitMembers(members)
			lists = append(lists, l)
		}
	}
	rows.Close()

	// A domain needs its virtual-domain entry if it has any mail object at all.
	domains := s.mailDomains()

	return system.RebuildMailMaps(s.Cfg.MailDir, domains, boxes, aliases, lists)
}

// mailDomains returns every domain that has a mailbox, alias or list, so Postfix
// accepts mail for it.
func (s *Server) mailDomains() []string {
	domains := []string{}
	rows, err := s.DB.Query(`SELECT DISTINCT d.name FROM domains d
		WHERE EXISTS (SELECT 1 FROM mailboxes m WHERE m.domain_id = d.id)
		   ...
		   OR EXISTS (SELECT 1 FROM mail_lists l WHERE l.domain_id = d.id)`)
	if err != nil {
		return domains
	}
	defer rows.Close()
...
func (s *Server) handleMailList(w http.ResponseWriter, r *http.Request, u *models.User) {
	...
	aliases := []models.MailAlias{}
	rows, err = s.DB.Query(`SELECT a.id, a.domain_id, a.source, a.destination, a.keep_copy
	...
	lists := []models.MailList{}
	rows, err = s.DB.Query(`SELECT l.id, l.domain_id, l.address, l.members, l.created_at
		FROM mail_lists l JOIN domains d ON d.id = l.domain_id WHERE `+where+` ORDER BY l.address`, args...)
	if err != nil {
		s.fail(w, "list mailing lists", err)
		return
	}
	for rows.Next() {
		var l models.MailList
		var members string
		if rows.Scan(&l.ID, &l.DomainID, &l.Address, &members, &l.CreatedAt) == nil {
			l.Members = splitMembers(members)
			lists = append(lists, l)
		}
	}
	...
	s.json(w, map[string]any{
		...
		"aliases":   aliases,
		"lists":     lists,
		"features":  system.MailFeaturesAvailable(),
		"imapsync":  system.HaveIMAPSync(),
	})
}

// validMailTarget validates a single destination address (real email, not a
// catch-all/local-part-only value).
func validMailTarget(addr string) bool {
	local, domain, ok := strings.Cut(strings.ToLower(strings.TrimSpace(addr)), "@")
	return ok && validMailLocalPart(local) && validDomainName(domain)
}

// parseTargets splits and validates a comma-separated destination list.
func parseTargets(dest string) ([]string, bool) {
	var out []string
...
func (s *Server) handleMailboxCreate(w http.ResponseWriter, r *http.Request, u *models.User) {
	...
	if msg := s.mailboxLimitReached(d.UserID); msg != "" {
...
func (s *Server) handleAliasCreate(w http.ResponseWriter, r *http.Request, u *models.User) {
	...
	// Source is either alias@domain or a catch-all @domain.
	local, domainName, ok := strings.Cut(source, "@")
	if !ok || !validDomainName(domainName) {
		s.err(w, http.StatusBadRequest, "invalid alias source")
		return
	}
	catchAll := local == ""
	if !catchAll && !validMailLocalPart(local) {
		s.err(w, http.StatusBadRequest, "invalid alias source")
		return
	}
	targets, ok := parseTargets(req.Destination)
	if !ok {
		s.err(w, http.StatusBadRequest, "destination must be one or more valid email addresses")
		return
	}
	d, ok := s.domainScopedByName(u, domainName)
	if !ok {
	...
	dest := strings.Join(targets, ", ")
	res, err := s.DB.Exec(`INSERT INTO mail_aliases(domain_id,source,destination,keep_copy) VALUES(?,?,?,?)`,
		d.ID, source, dest, boolInt(req.KeepCopy && !catchAll))
	if err != nil {
	...
	id, _ := res.LastInsertId()
	s.json(w, models.MailAlias{ID: id, DomainID: d.ID, Source: source, Destination: dest, KeepCopy: req.KeepCopy && !catchAll})
}
