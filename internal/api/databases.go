...
// dbPrefix returns the per-account identifier prefix (cPanel/Plesk style) that
// every database and database user created by an account is namespaced under,
// e.g. "alice_". It is derived from the panel username so each tenant's objects
// are clearly owned and cannot collide with another tenant's. The admin account
// is the server owner and is intentionally not prefixed.
func dbPrefix(u *models.User) string {
	if u.Role == models.RoleAdmin {
		return ""
	}
	p := nonAlnum.ReplaceAllString(strings.ToLower(u.Username), "_")
	p = strings.Trim(p, "_")
	if len(p) > 16 {
		p = p[:16]
	}
...
// applyDBPrefix prepends the account prefix to a user-supplied name unless it is
// already present, so typing the prefix (or re-using an existing prefixed name)
// never doubles it.
func applyDBPrefix(prefix, name string) string {
...
func (s *Server) handleDBCreate(w http.ResponseWriter, r *http.Request, u *models.User) {
	...
	if msg := s.databaseLimitReached(u.ID); msg != "" {
	...
	// Namespace the database and its user under the account's prefix (cPanel
	// style). Validation above runs on the typed portion; the prefix is trusted.
	prefix := dbPrefix(u)
	name = applyDBPrefix(prefix, name)
	dbUser = applyDBPrefix(prefix, dbUser)
	if len(name) > 64 || len(dbUser) > 64 {
		s.err(w, http.StatusBadRequest, "name is too long once the account prefix is added; use a shorter name")
		return
	}
