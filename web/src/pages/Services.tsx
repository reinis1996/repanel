...
import { Play, Square, RotateCw, Download, Loader2, CheckCircle2, FileText } from 'lucide-react'
import { api, useFetch } from '../api'
import type { ServiceStatus, PHPVersionInfo, ServiceHealth } from '../types'
import { Btn, Card, PageHeader, Table, Td, Modal, Spinner, ErrorBanner, Badge, toast } from '../components/ui'
...
export default function Services() {
  ...
  return (
    <div>
          ...
          {(data ?? []).map((s) => (
            <tr key={s.name} className="hover:bg-slate-50/60">
              <Td>
                ...
                {s.version && (
                  <span className="ml-2 text-xs text-slate-500 font-mono bg-slate-100 rounded px-1.5 py-0.5">v{s.version}</span>
                )}
              ...
              <Td className="text-right whitespace-nowrap">
                {s.installed && (
                  <Btn size="sm" variant="secondary" className="mr-2" title="Status & logs" onClick={() => setLogsFor(s)}>
      ...
      {logsFor && <ServiceLogsModal svc={logsFor} onClose={() => setLogsFor(null)} />}
    </div>
  )
}

function ServiceLogsModal({ svc, onClose }: { svc: ServiceStatus; onClose: () => void }) {
  const { data, loading, error } = useFetch<ServiceHealth>(`/api/services/${svc.name}/logs`)
  return (
    <Modal open title={`${svc.display_name} — status & logs`} onClose={onClose} wide>
      {loading ? (
        <Spinner />
      ) : error ? (
        <ErrorBanner message={error} />
        ...
        <div className="space-y-4">
          {data?.status && (
            <div>
              ...
              <pre className="max-h-44 overflow-auto rounded-md bg-slate-900 text-slate-100 text-xs font-mono p-3 whitespace-pre-wrap">{data.status}</pre>
            </div>
          )}
          <div>
            <div className="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-1">Recent log</div>
            <pre className="max-h-80 overflow-auto rounded-md bg-slate-900 text-slate-100 text-xs font-mono p-3 whitespace-pre-wrap">
              {data?.logs?.trim() || 'No recent journal entries.'}
            </pre>
          </div>
        </div>
      )}
